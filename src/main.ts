...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Erika Cobros API')
    .setDescription('Servicios para gestión y ejecución de cobros automatizados')
    .setVersion('1.0.0')
    .build();
  ...
  logger.log(
    `Aplicación erika-back-cobros corriendo en el puerto ${Config.puerto}`,
  );
}
bootstrap();
